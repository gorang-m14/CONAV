...
def main(args, send_queue, receive_queue):
    ...
    while count_episodes < num_episodes:
        ...
        # define these once so visualize block doesn't crash on first iteration
        ...
        while not env.episode_over:
            ...
            if (agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0) and not found_goal:
                ...
                if len(target_point_list) > 0 and agent[0].l_step > 0:
                    # Candidate maps (BytesIO list)
                    candidate_map_list = chat_utils.get_all_candidate_maps(
                        target_edge_map, top_view_map, pose_pred
                    )

                    # FIX: use agent[0].goal_name (your old code used agent[i] where i leaks from loop)
                    ...
                    # This function name remains chat_with_gpt4v, but in your edited chat_utils.py
                    # it calls MiniCPM locally.
        ...
        log += ", ".join(k + ": {:.3f}".format(v / count_episodes) for k, v in agg_metrics.items()) + \
